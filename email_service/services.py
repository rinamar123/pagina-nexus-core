...
def _send_email(subject, body, to_email):
    if settings.GMAIL_API_ENABLED:
        sent = send_gmail_api_message(to_email, subject, body)
        if sent:
            return True
...
-- SISTEMA NUCLEUS IA --
""".strip()
    return _send_email(subject, body, student.email)

def send_enrollment_rejected_email(student, course=None):
    subject = 'ACTUALIZACION DE ESTADO - IA ACADEMY'
...
def send_password_changed_email(student, reset_url):
    subject = 'ACTUALIZACION DE CREDENCIALES EXITOSA - IA ACADEMY'
    body = f"""
...
-- SISTEMA NUCLEUS IA --
""".strip()
    return _send_email(subject, body, student_email)
...
def send_student_suspended_email(student):
    subject = 'CUENTA SUSPENDIDA HASTA NUEVO AVISO - IA ACADEMY'
    body = f"""
